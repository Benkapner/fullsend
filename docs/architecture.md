...
- Cross-org mint authorization: workflows may request tokens for a different org via optional `target_org` when the target org installs the role App and sets `FULLSEND_FOREIGN_<role>_REPOS`. Foreign (cross-org) mints require empty `repos` (or `["*"]` as an empty alias) and FOREIGN gating. Same-org mints must scope `repos` to the requesting repository by default; `PER_ORG_FOREIGN_COMPAT` unlocks org-mode shapes (broad non-empty lists from `.fullsend`, or `[.fullsend]` / `{self,.fullsend}` from enrolled callers). Same-org installation-wide tokens are denied ([ADR 0060](ADRs/0060-cross-org-mint-authorization-via-org-variables.md)).
