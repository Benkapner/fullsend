...
| `--status-auth` | `oidc` | Comma-separated status auth modes. Each non-oidc mode selects a Go build tag. Modes: `oidc`, `github`. `oidc` is always compiled in; `github` requires `--status-github-group` and `--status-github-client-id` |
...
| `--status-github-client-id` | | GitHub OAuth App client ID for status auth (required when `github` mode enabled) |
