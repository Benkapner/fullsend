...
- **`mintEnv` and `mintHTTP` are package-internal accessors** in `internal/mintcore/`. `NewHandler` reads configuration via `mintEnv(key)` and obtains its GitHub API HTTP client via `mintHTTP()`. On native platforms (`//go:build !js`), `mintEnv` delegates to `os.Getenv` and `mintHTTP` returns a cached `*http.Client`. On WASM (`//go:build js`), the CF Worker calls `RegisterEnv` and `RegisterHTTP` once during `mintcoreInitMint` to supply JS callbacks. **Do not** call `mintEnv` or `mintHTTP` inside heavy verifier constructor files (`jwks_verifier.go`, `sts_verifier.go`) — this pulls the entire env/HTTP dependency graph into the WASM binary. Tests override the HTTP client with `SetHTTPDoerForTest(t, fake)` and use `t.Setenv` for environment variables.
