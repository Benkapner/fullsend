...
**Not adopted as written.** Blocked by [#5756](https://github.com/fullsend-ai/fullsend/issues/5756) —
pre-script env vars do not propagate to the sandbox. See Decision for the
revised variant that moves metadata extraction inside the sandbox.
...
**Skill loading** follows the same skill-concatenation pattern currently used by
the `docs-currency` sub-agent: the orchestrator reads both the sub-agent `.md`
and `skills/pr-risk-assessment/SKILL.md`, concatenates them into the prompt.
This requires generalizing the orchestrator's existing hardcoded
`docs-currency` branch into a per-sub-agent skill lookup. When sub-agents gain
isolated sandboxes ([#3978](https://github.com/fullsend-ai/fullsend/issues/3978),
...
**Post-review integration:** `post-review.sh` (in
[fullsend-ai/agents](https://github.com/fullsend-ai/agents), `scripts/post-review.sh`)
reads the risk assessment from the result JSON, applies a `risk/*` label
(removing any prior `risk/*` label), and appends a breakdown table to the PR
comment. Risk level is informational only — it does not gate the review outcome.
The protected-path check remains the sole blocking mechanism.
...
- Risk scoring adds one sonnet-model sub-agent call per PR. Sonnet is the
  initial model choice based on the classification nature of the task; cost and
  accuracy should be validated after production data is available.
...
- The skill-concatenation pattern (generalized from the existing `docs-currency`
  branch) adds an implicit coupling between the orchestrator and the skill
  directory; this coupling is eliminated when sub-agent sandbox isolation lands
