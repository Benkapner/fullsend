...
- Verifier backends become thin wrappers: parse and verify the token, then call `AuthorizeToken` + `ValidateWorkflowRef`. Policy changes require editing one function.
...
- Public mints should migrate `ALLOWED_ORGS=*` to `PER_REPO_WIF_REPOS=*` (or add `*` to `PER_REPO_WIF_REPOS`). The legacy `IsPublicMint(allowedOrgs)` function remains for backward compatibility but is deprecated.
