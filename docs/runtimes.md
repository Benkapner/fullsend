...
Fullsend's `fullsend run` command delegates in-sandbox agent execution to a pluggable **runtime**. Recognized values in org `config.yaml` `defaults.runtime` (and per-repo `runtime`) are **`claude`** (production default), **`pi`** (opt-in, #6464) and **`dummy`** (behaviour tests only). Install with `fullsend admin install --runtime pi` or `--runtime dummy` (dedicated test orgs only), or set `defaults.runtime: pi` in the org config / `runtime: pi` in a repo's `.fullsend/config.yaml` for an existing install — no image or workflow change is needed, the sandbox image ships both CLIs. The runner resolves the backend via `runtime.ResolveFromConfig()` after loading the org config and prints `runtime: selected "<name>"` at the start of every run.
