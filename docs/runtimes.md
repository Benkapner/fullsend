...
| **Prompt injection (DeBERTa)** | Host `fullsend scan input` only (not invoked by `fullsend run`) | ✗ — compiled out of every shipped build; a working build needs `CGO_ENABLED=1 -tags ORT` + `libtokenizers.a` (#6522) | N/A — stub | ✗ — same binary, same constraint | Not an active control in any shipped image. Source (`mlscan_ort.go`) retained; re-enabling also means re-adding the model + ONNX Runtime to the runner image. See #6506 (decision), #6522 (build constraints) |
