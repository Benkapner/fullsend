...
| **Prompt injection (DeBERTa)** | Host `fullsend scan input` only (compiled out; not invoked by `fullsend run`) | ✗ — compiled out of all shipped builds (`CGO_ENABLED=0`, no `-tags ORT`); `MLScanAvailable()` returns true with the ORT tag but the ORT session fails without cgo + `libtokenizers.a`, so `RunMLScan` fail-opens. Available only via a local `-tags ORT` build with `CGO_ENABLED=1` + `libtokenizers.a` | N/A — stub | ✗ — same binary, same constraint | Not an active control in any shipped image, and not on the `fullsend run` path at all — `RunMLScan` is called only from `fullsend scan input` (`internal/cli/scan.go`), never from Path A (`scanRepoContextFiles`) or Path B (`fullsend scan context`). See #6506 (option B decision) and #6522 (build constraints). The Go source (`internal/security/mlscan_ort.go`) is retained, but re-enabling now also means re-adding the model and ONNX Runtime to the runner image |
