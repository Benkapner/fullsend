...
| **Prompt injection (DeBERTa)** | Host Path A (compiled out) | ✗ — compiled out of all shipped builds (`CGO_ENABLED=0`, no `-tags ORT`); `MLScanAvailable()` returns true with the ORT tag but the ORT session fails without cgo + `libtokenizers.a`, so `RunMLScan` fail-opens. Available only via a local `-tags ORT` build with `CGO_ENABLED=1` + `libtokenizers.a` | N/A — stub | ✗ — same binary, same constraint | Not an active control in any shipped image. See #6506 (option B decision) and #6522 (build constraints). The Go source (`internal/security/mlscan_ort.go`) is retained so the option is one build flag away |
