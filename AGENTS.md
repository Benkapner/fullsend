...
- When extending a Go interface with new methods, check `docs/guides/dev/` for documentation that enumerates the interface's methods (e.g., `behaviour-drivers.md` lists `ci.Driver` methods). If found, update the method list to include all current methods. The `lint-interface-doc-sync` pre-commit hook enforces known pairs.
